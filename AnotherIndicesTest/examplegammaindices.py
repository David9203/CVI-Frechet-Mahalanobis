import sys
sys.path.insert(0,'/home/nestord.rendon/nestor/CVI-Frechet-Mahalanobis/Anotherindicestest/mdcgenpy/mdcgenpy')
...
sys.path.insert(1,'/home/nestord.rendon/nestor/CVI-Frechet-Mahalanobis/Anotherindicestest/mdcgenpy/mdcgenpy/Clusters-Features')
...
def iterateindex(inf,sup):
  ...
  for index, row in car.iloc[inf:sup,3:12].iterrows():
    ...
    for l in range(2,kgmm):
      ...
      GM = GaussianMixture(n_components=l, random_state=rand, covariance_type="full").fit(X_train11.values)
      pred=GM.predict(X_train11.values)
    ...
    for i in range(len(ncrecomended)):
      GMM_test= GaussianMixture(n_components=ncrecomended[i][0], covariance_type="full",random_state=1).fit(X_train11.iloc[:,:row.n_feats].values)
      predy=GMM_test.predict(X_train11.iloc[:,:row.n_feats].values)
...
inf=0
sup=50
pathfile="/home/nestord.rendon/nestor/CVI-Frechet-Mahalanobis/Anotherindicestest/SyntheticData/info_generateddata_gamma.xlsx"
car=pd.read_excel(pathfile)
path="/home/nestor/gamma/"
